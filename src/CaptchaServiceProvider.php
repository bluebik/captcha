    ...
    public function boot()
    {
        ...
        if (strpos($this->app->version(), 'Lumen') !== false) {
            ...
            if ((double)$this->app->version() >= 5.2) {
                $this->app['router']->get(config('captcha.version_path') . 'captcha/api/{config?}', '\Mews\Captcha\CaptchaController@getCaptchaApi')->middleware('web');
                $this->app['router']->get(config('captcha.version_path') . 'captcha/{config?}', '\Mews\Captcha\CaptchaController@getCaptcha')->middleware('web');
            } else {
